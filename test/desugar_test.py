...
from yarpen.desugar import Desugarer
import unittest
...
class DesugarTest(TestCase):
    ...
    def setUp(self):
        self.desugar = Desugarer()

    def test_desugar_empty_let(self):
        expr = Parser("(let () 3)").parse()[0]
        desugared_exp = self.desugar.transform(expr)
    ...
    def test_desugar_nested_let(self):
        ...
        desugared_expr = self.desugar.transform(expr)
    ...
    def test_desugar_let_start(self):
        ...
        desugared_exp = self.desugar.transform(expr)
        expected = self.desugar.transform(Parser("(let ((x 1)) (let ((y (+ x 1))) y))").parse()[0])
        self.assertEqual(desugared_exp, expected)
    ...
    def test_desugar_letrec(self):
        ...
        desugared = self.desugar.transform(expr)
